...
// Products
exports.productTable = db_1.connection.query("CREATE TABLE IF NOT EXISTS Products (ProductId INTEGER UNSIGNED PRIMARY KEY AUTO_INCREMENT NOT NULL,  ProductName VARCHAR(50) NOT NULL, ProductModel VARCHAR(50) NOT NULL, ProductDescription TEXT, Status ENUM('a','i') NOT NULL, createdAt TIMESTAMP NOT NULL, updatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(), UserId INTEGER UNSIGNED NOT NULL, CategoryId INTEGER UNSIGNED NOT NULL, FOREIGN KEY (UserId) REFERENCES Users(UserId), FOREIGN KEY (CategoryId) REFERENCES Categories(CategoryId))", function (err, results, fields) {
    if (err)
        throw err;
    console.log("Table created"); // results contains rows returned by server
    // console.log(fields); // fields contains extra meta data about results, if available
